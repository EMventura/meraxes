        ...
        if (snapshot - snapshot_counter_backwards[R_ct] == 0 && weight < 0) {
          ...
          while (zpp_edge[R_ct] > run_globals.ZZ[snapshot - snapshot_counter_backwards[R_ct]]) {
            ...
            if (zpp_edge[R_ct] < run_globals.ZZ[snapshot - snapshot_counter_backwards[R_ct] - 1]) {
              zedge = zpp_edge[R_ct - 1]
                        ? run_globals.ZZ[snapshot - snapshot_counter_backwards[R_ct]]
                        : zpp_edge[R_ct - 1] > run_globals.ZZ[snapshot - snapshot_counter_backwards[R_ct]];
    ...
    if (collapse_fraction > 0.0) {
      ...
      filling_factor_of_HI_zp = 1. - ReionEfficiency * collapse_fraction / (1.0 - x_e_ave);
        ...
        if (filling_factor_of_HI_zp < 0)
          filling_factor_of_HI_zp = 0; // for global evol; nu_tau_one above treats negative (post_reionization) inferred
                                       // filling factors properly
        ...
        if (R_ct > 2 && sum_lyn[R_ct] == 0.0 && sum_lyn[R_ct - 1] > 0. && first_radii) {
          ...
          // Split the previous radii and current radii into n_pts_radii smaller radii (redshift) to have fine control
          // of where it transitions from zero to non-zero This is a coarse approximation as it assumes that the linear
      ...
      if (fabs(run_globals.params.physics.SpecIndexXrayGal - 1.0) < 0.000001) {
        ...
        Luminosity_converstion_factor_GAL = 1. / Luminosity_converstion_factor_GAL;
        Luminosity_converstion_factor_GAL *= pow(run_globals.params.physics.NuXrayGalThreshold * NU_over_EV,
                                                 -run_globals.params.physics.SpecIndexXrayGal) *
                                             (1 - run_globals.params.physics.SpecIndexXrayGal);
      }
      ...
      if (fabs(run_globals.params.physics.SpecIndexXrayIII - 1.0) < 0.000001) {
        ...
        Luminosity_converstion_factor_III = 1. / Luminosity_converstion_factor_III;
        Luminosity_converstion_factor_III *= pow(run_globals.params.physics.NuXrayGalThreshold * NU_over_EV,
                                                 -run_globals.params.physics.SpecIndexXrayIII) *
                                             (1 - run_globals.params.physics.SpecIndexXrayIII);
      }
      ...
      // Had issues with normalisation factors comparing the codes, adding 0.59 here rectified the normalisation
      // somewhat The Lya background appeared to be a factor of ~ 2 higher than 21cmFAST for the same luminosity. Spent
      // days searching for the difference, this seems to explain it. Can either boost the luminosity conversion, or
      // lower the lya background by the same factor in XRayHeatingFunctions.c (evolveInt). Note: When comparing to
      // 21cmFAST it is important that this factor is included! Will mean the normalisation within Meraxes is (1/0.59)
      // higher than 21cmFAST, which can be trivially compensated for by reducing L_X. Ultimately the backgrounds in
      // Meraxes will be this same factor higher than 21cmFAST, but at least it is understood why and trivially
      // accounted for.
      ...
      for (int ix = 0; ix < local_nix; ix++)
        for (int iy = 0; iy < ReionGridDim; iy++)
          for (int iz = 0; iz < ReionGridDim; iz++) {
            ...
            // Perform the calculation of the heating/ionisation integrals, updating relevant quantities etc. GET BACK
            // AT THIS FOR USE_MINI_HALOS!!
#if USE_MINI_HALOS || USE_SCALING_REL
            evolveInt((float)zp,
                      run_globals.reion_grids.deltax[i_padded],
                      SFR_GAL,
