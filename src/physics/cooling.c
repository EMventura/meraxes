...
void cool_gas_onto_galaxy(galaxy_t* gal, double cooling_mass)
{
  ...
  if (cooling_mass >= 1e-10) {
    ...
    add_disks(gal, 1, cooling_mass, vector_magnitude(newAngMom) / (2 * gal->Vvir), gal->Vvir, AMcool);
  ...
  if ((3 * gal->DiskScaleLength > gal->Rstar) && (gal->Rstar > 0.)) {
    ...
    if (frac < 0.0) {
      ...
      mlog("strange value of frac = %.3f with Rstar = %f and Rdisk = %f",
           MLOG_MESG,
           frac,
           gal->Rstar,
           3 * gal->DiskScaleLength);
      frac = 0.0;
    }
    if (frac > 1.0)
      frac = 1.0;
  ...
  /*increment_angular_momentum(gal->AMcold, gal->Halo->AngMom,
                             cooling_mass);*/
  if (cooling_mass >= 1e-10)
    increment_angular_momentum(gal->AMcold, newAngMom, cooling_mass);
#endif
}
